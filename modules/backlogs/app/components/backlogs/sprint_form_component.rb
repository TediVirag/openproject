  ...
  class SprintFormComponent < ApplicationComponent
    ...
    def form_url
      if sprint.new_record?
        project_backlogs_sprints_path(project.id, all_backlogs_params)
      else
        project_backlogs_sprint_path(project.id, sprint.id, all_backlogs_params)
      end
    end
    ...
    def data_attributes
      {
        ...
        "refresh-on-form-changes-turbo-stream-url-value":
          refresh_form_project_backlogs_sprints_path(project.id, all_backlogs_params)
      }
    end
  end
end
